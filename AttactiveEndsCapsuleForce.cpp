...
void AttractiveEndsCapsuleForce<ELEMENT_DIM, SPACE_DIM>::SetYoungModulus(double youngModulus)
...
template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double AttractiveEndsCapsuleForce<ELEMENT_DIM, SPACE_DIM>::GetYoungModulus()
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double AttractiveEndsCapsuleForce<ELEMENT_DIM,SPACE_DIM>::CalculateOverlapBetweenCapsules(
...
template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double AttractiveEndsCapsuleForce<ELEMENT_DIM, SPACE_DIM>::CalculateForceDirectionAndContactPoints(Node<SPACE_DIM>& rNodeA,
                                                                                     Node<SPACE_DIM>& rNodeB,
...
//        PRINT_VECTOR(w);
	...
	if ((fabs(sc-0.5)>0.40) && (fabs(tc-0.5)>0.40))
	{
		double Overlap = CalculateOverlapBetweenCapsules(rNodeA, rNodeB, shortest_distance); // return the overlap
		return Overlap;
	}
	else
	{
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
double AttractiveEndsCapsuleForce<ELEMENT_DIM,SPACE_DIM>::CalculateForceMagnitude(const double overlap,
                                                                    ...
                                                                    const double radiusB)
{
    ...
    if (overlap > radiusA || (overlap > -0.1 && overlap < -2.0*radiusA) ) // Changed this to 0.0 as opposed to radiusA - as the attractive ends force needs to work when the objects are not touching eachother?
    {
    	return 0.0;
    }
    else
...
template<unsigned ELEMENT_DIM, unsigned SPACE_DIM>
void AttractiveEndsCapsuleForce<ELEMENT_DIM,SPACE_DIM>::AddForceContribution(AbstractCellPopulation<ELEMENT_DIM,SPACE_DIM>& rCellPopulation)
{
...
template class AttractiveEndsCapsuleForce<1,1>;
template class AttractiveEndsCapsuleForce<1,2>;
template class AttractiveEndsCapsuleForce<2,2>;
template class AttractiveEndsCapsuleForce<1,3>;
template class AttractiveEndsCapsuleForce<2,3>;
template class AttractiveEndsCapsuleForce<3,3>;
