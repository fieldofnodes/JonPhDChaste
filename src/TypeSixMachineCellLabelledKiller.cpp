...
TypeSixMachineCellLabelledKiller<DIM>::TypeSixMachineCellLabelledKiller(AbstractCellPopulation<DIM>* pCellPopulation)
    : AbstractCellKiller<DIM>(pCellPopulation)
{
}

template<unsigned DIM>
void TypeSixMachineCellLabelledKiller<DIM>::SetCellTypeLabelStatus(bool CellTypeLabelStatus)
{
	mCellTypeLabelStatus=CellTypeLabelStatus;
}


template<unsigned DIM>
bool TypeSixMachineCellLabelledKiller<DIM>::GetCellTypeLabelStatus()
{
	return mCellTypeLabelStatus;
}


template<unsigned DIM>
void TypeSixMachineCellLabelledKiller<DIM>::CheckAndLabelCellsForApoptosisOrDeath()
{
         ...
         ++cell_iter)
    {
        ...
        {
            EXCEPTION("TypeSixMachineCellLabelledKiller cannot be used unless each cell has a TypeSixMachineProperty");
        }
        ...
        for (auto& r_pair : r_data)
        {
            ...
            if (state == 3u)
            {
	            ...
	            //Here do the cell_iter and get the typesixpropertu collection - get property then get the type
                     ...
                     ++it)
                {
					...
					CellPtr p_cell = p_population->GetCellUsingLocationIndex(p_neighbour->GetIndex());



                    //Get the cell corresponding to the node -- then use that pointer to the cell and get the property and if the tpes are the same dont kill
				    ...
				    // Logic regarding the cell types kill here is where i use the get and set method for the status of the cell types
				    // For now just do the cell types are the same
...
template<unsigned DIM>
void TypeSixMachineCellLabelledKiller<DIM>::OutputCellKillerParameters(out_stream& rParamsFile)
{
    AbstractCellKiller<DIM>::OutputCellKillerParameters(rParamsFile);
}

// Explicit instantiation
template class TypeSixMachineCellLabelledKiller<1>;
template class TypeSixMachineCellLabelledKiller<2>;
template class TypeSixMachineCellLabelledKiller<3>;
